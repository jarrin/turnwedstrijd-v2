...
const SECRETARIAT_GENDER_FILTER_STORAGE_KEY = 'secretariat.genderFilter';
...
const secretariatGenderFilter = document.getElementById('secretariatGenderFilter');
...
secretariatGenderFilter.addEventListener('change', handleGenderFilterChange);
...
async function refreshScores() {
    ...
    const filteredScores = getGenderFilteredScores(allPendingScores, currentGenderFilter);

    if (filteredScores.length === 0) {
        currentScoresPage = 1;
        container.innerHTML = '<p class="empty-state">Geen wachtende scores voor dit filter</p>';
        return;
    }
    ...
    const visibleScores = filteredScores.slice(startIndex, endIndex);
    ...
    if (filteredScores.length > SCORES_PER_PAGE) {
        ...
        html += `
            <div class="pagination-bar pagination-bar-compact">
                <div class="pagination-info">
                    Toon ${startIndex + 1}-${Math.min(endIndex, filteredScores.length)} van ${filteredScores.length} wachtende scores
                </div>
...
function initializeGenderFilter() {
    const savedFilter = localStorage.getItem(SECRETARIAT_GENDER_FILTER_STORAGE_KEY) || '';
    if (savedFilter === 'Heren' || savedFilter === 'Dames') {
        currentGenderFilter = savedFilter;
        secretariatGenderFilter.value = savedFilter;
        return;
    }

    currentGenderFilter = '';
    secretariatGenderFilter.value = '';
    localStorage.removeItem(SECRETARIAT_GENDER_FILTER_STORAGE_KEY);
}
...
function handleGenderFilterChange(event) {
    ...
    if (selected !== 'Heren' && selected !== 'Dames') {
        ...
        localStorage.setItem(SECRETARIAT_GENDER_FILTER_STORAGE_KEY, selected);
    }
...
function getGenderFilteredScores(scores, genderFilter) {
    if (genderFilter !== 'Heren' && genderFilter !== 'Dames') {
        return scores;
    }

    return scores.filter((score) => score.gender === genderFilter);
}
